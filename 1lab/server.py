    ...
    while not shutdown_event.is_set() and not program_controls[program_name]["stop_event"].is_set():  # Цикл до завершения работы
        ...
        try:
            ...
            if program_controls[program_name]["verbose"].is_set():
                if LOGGING:
                    logging.info(f'[OUTPUT] [{program_name}] : {output}')  # Отправляем сообщение клиенту
            ...
            if cmd == "ADD" and len(parts) >= 2:  # Если команда ADD
                ...
                if len(parts)>=3:
                    ...
                    if prog not in program_controls:  # Если программы нет в списке
                        ...
                        if program_controls[prog]["stop_event"].is_set():  # Если программа не работает
                            response = f"Программа '{prog}' не работает"  # Формируем сообщение
                        else:
                            if strtobool(parts[1]):
                                response = f"Циклические выводы программы '{prog}' начали осуществляться "  # Формируем сообщение
                                program_controls[prog]["verbose"].set()
                            else:
                                response = f"Циклические выводы программы '{prog}' прекратили осуществляться "  # Формируем сообщение
                                program_controls[prog]["verbose"].clear()
                    
                else:
                    response = ''
                    for element in program_controls.keys():
                        if program_controls[element]["stop_event"].is_set():  # Если программа не работает
